...
Корректное именование полей формы на любой глубине обеспечивает нативный `MoonShine.iterable.reindex()` — поле лишь проставляет `data-row-key` и dot-path (`flPath`) для адресации вложенных макетов в AJAX-запросах. `flPath` проставляется рекурсивно: вложенный `FlexibleLayouts` внутри `Flex`/`Column`/`FieldsGroup` адресуется так же корректно, как и вложенный напрямую.
