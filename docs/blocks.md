...
При `limit: N` сервер считает инстансы типа по **сохранённой модели** (authoritative) и отклоняет превышение тостом «Limit count N». Для ещё не сохранённых записей используется счётчик из запроса. Проверка выполняется в `BlockController::store()` — обход лимита через прямой запрос невозможен. Дублирование блока проходит через тот же эндпоинт — лимит соблюдается и при дублировании.
